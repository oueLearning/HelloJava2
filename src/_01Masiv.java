 ...
 */
public class _01Masiv {
    public static void main(String[] args) {


        // масив это набор элементов одного типа
        //с произвольным доступом (произвольный доступ означает что мы можем обратится к 3, потом к 1 потом к пятому элементу или
        // вообще как угодно, не зависимо от времени) например в линкед листах чтоб получить 3й элемент мы должны знать адресс первого
        // элемента у которого берем адрес 2го элемента а у него берем адрес 3го и только тогда используем его. Это не произвольный доступ
        ...
        _01Masiv[] masivObjektov; // масив обьектов класса Masiv
        int [][] masivMasivovIntov; // масив масивов, каждая ячейка масива хранит ссылку на другой масив в котором хранятся инты
        ...
        // (1 бит ячейка памяти которая может хранить одно значение 0 или 1, есть ток или нет тока)
        ...
        //по этому значение элемента с индексом 1 будет находиться между 4тым и 9тым байтами (5,6,7,8) памяти выделеной для масива
...
//        горячие клавиши если хотим закоментеть или раскоментить строчку нажимаем ctrl+деление(строку выделять не обязательно, если несколько строк тогда выделяем)
